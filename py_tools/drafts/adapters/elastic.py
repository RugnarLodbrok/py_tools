...
class ElasticAdapter:
    ...
    def __init__(self, index, url=None, doc_type='doc', fields=None, timestamp_int=False):
        ...
        if fields is None:
            fields = dict(timestamp=dict(type="date", format="epoch_second||yy-MM-dd HH:mm:ss.SSSSSSZ"))
    ...
    def _add_index(self, index):
        ...
        for i in range(4):  # assuming we can create index in 4 tries or find it created by another client
            ...
            raise AssertionError("couldn't create an elastic index in 4 tries: {}".format(index))
    ...
    def post(self, json, **kwargs):
        ...
        r = http_session.session.post(self.url + "/" + index + "/" + self.doc_type, json=json, **kwargs)
        if not r.ok:
            print(url)
