...
class Command:
    def __init__(self, cmd, timeout=2 * 60 * 60, wait=True, out_log=None,
                 silent=False,
                 separate_stdout=False, write_stdout=True, dt=10,
                 track_resources=False, retry=3):
        """
    ...
    def execute(self, raise_=True, shell=False):
        ...
        p = self._popen(cmd, stdout=self.out_log or PIPE, stderr=self.out_log or PIPE, shell=shell)
        thread = Thread(target=subprocess_thread)  # run subprocess in this thread
        thread.start()
        if not self.out_log:
            Thread(target=stdout_thread, args=(p.stdout, self._stdout)).start()  # capture stdout
            Thread(target=stdout_thread,
                   args=(p.stderr, self._stderr if self._separate_stdout else self._stdout)).start()  # capture stderr
            ...
            if thread.is_alive():
                ...
                if self.track_resources:
                    ...
                    try:
                        ...
                        self._memory_usage.append(p.memory_info().vms + sum(c.memory_info().vms for c in p.children()))
                ...
                ret_code = p.poll()
                self.logger.info("command finished", cmd=cmd, time=self.time, parameters=params,
                                 unit='seconds', return_code=ret_code)
                self._finished = True
                if ret_code and raise_:
                    raise CommandExecution(argv0, parameters=params, ret_code=ret_code, stdout=self.stdout())
                return self
        else:
            self.logger.info("command timed out", cmd=cmd, time=self.timeout, unit='seconds')
            p.terminate()
            thread.join()
            self._finished = True
            raise ExecutionTimeout(argv0, parameters=params, timeout=self.timeout, stdout=self.stdout())
...
class DockerCommand(Command):
    ...
    def _popen(self, *args, **kwargs):
        ...
        c_login = 'docker login --username {} --password {} {}'.format(self.user or self.USER,
                                                                       self.passwd or self.PASSWD, img)
        c_pull = 'docker pull {}'.format(img)
        c_run = 'docker run -v {mnt_src}:{mnt_dst} -w {wd} {img} {cmd}'.format(
            mnt_src=os.getcwd(), mnt_dst='/wd',
            wd="/wd", img=img, cmd=cmd)
        check_call(c_login, shell=True)
        check_call(c_pull, shell=True)
        return Popen(c_run, shell=True, stdout=self.out_log or PIPE, stderr=self.out_log or PIPE)
