import glob
import os
from os.path import join as j, isfile, isdir, exists
...
from py_tools.common import retry as retry_dec, short_md5
...
def handle_remove_readonly(func, path, exc):
    ...
    if func in (os.rmdir, os.remove, os.unlink) and exc_value.errno == errno.EACCES:
        os.chmod(path, stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO | stat.S_IWRITE)  # 0777 + remove readonly flag
        func(path)
    elif func in (os.rmdir, os.remove, os.listdir, os.unlink) and exc_value.errno == errno.ENOENT:
        logger.info("Tried to {} `{}', but path does not exists".format(func.__name__, path))
    else:
        logger.info("Error in func {} with errno {}".format(func.__name__, exc_value.errno))
        raise exc_class(str(exc_value) + str(exc_value.errno)).with_traceback(tb)
        # raise exc_value


def walk_files(root, patterns='*', antipatterns='', recursive=True, yield_folders=False):
    """
    ...
    for path, subdirs, files in os.walk(root):
        ...
        files.sort()
        for name in files:
            if any(fnmatch(name, p) for p in patterns) and not any(fnmatch(name, p) for p in antipatterns):
