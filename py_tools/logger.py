...
class Logger:
    ...
    FILE_LOG_FORMAT = lg.Formatter('%(asctime)s %(process)d %(funcName)s [%(levelname)s]: %(message)s')
    STREAM_LOG_FORMAT = lg.Formatter('%(asctime)s [%(process)d]: %(message)s', datefmt='%d %H:%M:%S')
...
def basic_setup():
    ...
    global _logger
    h = lg.StreamHandler()
    h.setFormatter(StructFormatter('%(asctime)s [%(process)d]: %(message)s', datefmt='%d %H:%M:%S'))
