import sys
from concurrent.futures import ThreadPoolExecutor, Future, TimeoutError as FutureTimeoutError, ProcessPoolExecutor
...
def task_queue(f, iterator, pool=10, on_fail=lambda _: None, result_queue=None, mode='thread'):
    """
...
def threaded_generator(f, iterator, pool=10, on_fail=lambda _: None, q_size=5000, mode='thread'):
    result = Queue(maxsize=q_size)
    future = task_queue(f, iterator, pool=pool, on_fail=on_fail, result_queue=result, mode=mode)
    try:
        while not future.done():
            try:
                future.result(.2)
